...
from fractureSV_gui import Ui_MainWindow
...
class MyFirstGuiProgram(Ui_MainWindow):
    def __init__(self, dialog):
        ...
        self.fig_anray.subplots_adjust(hspace=0.27, left=0.10, right=0.92, top=0.93, bottom=0.17)
        self.fig_anray2.subplots_adjust(hspace=0.27, left=0.10, right=0.98, top=0.93, bottom=0.17)
        #subplots
        self.axes_anray_tot = self.fig_anray.add_subplot(511)
        ...
        self.axes_anray_rad = self.fig_anray.add_subplot(512)
        ...
        self.axes_anray_z = self.fig_anray.add_subplot(514)
        ...
        self.axes_anray_time = self.fig_anray.add_subplot(515)
        self.axes_anray2_time = self.fig_anray2.add_subplot(414)
        self.axes_anray_trans = self.fig_anray.add_subplot(513)
        ...
        # self.spinBox_vp1.setValue(2973)
        ...
        if not os.path.exists('HTI_SV_model'):
            os.makedirs('HTI_SV_model')
    ...
    def plot(self):
        ...
        A1 = -(deltaZ/ medZ) / 2
        B1 = ((7/2)*(deltavs/medvs)+2*deltap/medp)*pow(np.sin(theta * np.pi / 180), 2)
        C1 = -0.5* (deltavs / medvs) * pow(np.sin(theta * np.pi / 180), 2)*pow(np.tan(theta * np.pi / 180), 2)

        A2 = -((deltaZ/ medZ)-deltay) / 2
        B2 = ((7/2)*((deltavs/medvs)-deltay)+2*deltap/medp +0.5*((medvp/medvs)**2)*(deltae-deltad))* pow(np.sin(theta * np.pi / 180), 2)
        C2 = -0.5* ((deltavs / medvs)-deltay) * pow(np.sin(theta * np.pi / 180), 2)*pow(np.tan(theta * np.pi / 180), 2)
    ...
    def anray_thr(self, azh):
        ...
        status = p.wait()
        shutil.copy2('fort.30', '%s/HTI_SV_model/amplitudes_%s.dat' %(self.anray_path, azh))
        shutil.copy2('lu2.anray', '%s/HTI_SV_model/lu2_%s.anray' % (self.anray_path, azh))
        shutil.move('modelo_anray_%s.modelo' % azh,'%s/HTI_SV_model/modelo_anray_%s.modelo' % (self.anray_path, azh))
    ...
    def split(self):
        ...
        f2_0 = open('lu2_0.anray', "r")
        f2_30 = open('lu2_30.anray', "r")
        f2_45 = open('lu2_45.anray', "r")
        f2_60 = open('lu2_60.anray', "r")
        f2_90= open('lu2_90.anray', "r")
        time_basalto = []
        time_solo=[]
        geofone_0 = []
        x_0 = []
        ...
        for line in f2_0:
            ...
            if coluna[0] == var2:
                par = True
            if len(coluna[0]) == 72 and par==True:
                h = [line[i:i + 12] for i in range(0, len(coluna[0]), 12)]
                a= [float(i) for i in h]
                qs2_x0.append(np.sqrt(a[0]**2 + a[1]**2))
                qs2_y0.append(np.sqrt(a[2] ** 2 + a[3] ** 2))
                qs2_z0.append(np.sqrt(a[4] ** 2 + a[5] ** 2))

        f2_0.close()
        qs2_x0= np.asarray(qs2_x0)
        qs2_y0 = np.asarray(qs2_y0)
        trans = np.sqrt(qs2_x0**2+qs2_y0**2)
        trans_0 = np.fliplr([trans])[0]
        ...
        for line in f2_30:
            ...
            if len(coluna[0]) == 72 and par == True:
                ...
                if len(coluna[0]) == 72:
                    h = [line[i:i + 12] for i in range(0, len(coluna[0]), 12)]
                    a = [float(i) for i in h]
                    qs2_x30.append(np.sqrt(a[0] ** 2 + a[1] ** 2))
                    qs2_y30.append(np.sqrt(a[2] ** 2 + a[3] ** 2))
                    qs2_z30.append(np.sqrt(a[4] ** 2 + a[5] ** 2))

        f2_30.close()
        qs2_x30 = np.asarray(qs2_x30)
        qs2_y30 = np.asarray(qs2_y30)
        trans = np.sqrt(qs2_x30 ** 2 + qs2_y30 ** 2)
        trans_30 = np.fliplr([trans])[0]
        ...
        for line in f2_45:
            ...
            if len(coluna[0]) == 72 and par == True:
                ...
                if len(coluna[0]) == 72:
                    h = [line[i:i + 12] for i in range(0, len(coluna[0]), 12)]
                    a = [float(i) for i in h]
                    qs2_x45.append(np.sqrt(a[0] ** 2 + a[1] ** 2))
                    qs2_y45.append(np.sqrt(a[2] ** 2 + a[3] ** 2))
                    qs2_z45.append(np.sqrt(a[4] ** 2 + a[5] ** 2))

        f2_45.close()
        qs2_x45 = np.asarray(qs2_x45)
        qs2_y45 = np.asarray(qs2_y45)
        trans = np.sqrt(qs2_x45 ** 2 + qs2_y45 ** 2)
        trans_45 = np.fliplr([trans])[0]
        ...
        for line in f2_60:
            ...
            if len(coluna[0]) == 72 and par == True:
                ...
                if len(coluna[0]) == 72:
                    h = [line[i:i + 12] for i in range(0, len(coluna[0]), 12)]
                    a = [float(i) for i in h]
                    qs2_x60.append(np.sqrt(a[0] ** 2 + a[1] ** 2))
                    qs2_y60.append(np.sqrt(a[2] ** 2 + a[3] ** 2))
                    qs2_z60.append(np.sqrt(a[4] ** 2 + a[5] ** 2))

        f2_60.close()
        qs2_x60 = np.asarray(qs2_x60)
        qs2_y60 = np.asarray(qs2_y60)
        trans = np.sqrt(qs2_x60 ** 2 + qs2_y60 ** 2)
        trans_60= np.fliplr([trans])[0]
        ...
        for line in f2_90:
            ...
            if len(coluna[0]) == 72 and par == True:
                ...
                if len(coluna[0]) == 72:
                    h = [line[i:i + 12] for i in range(0, len(coluna[0]), 12)]
                    a = [float(i) for i in h]
                    qs2_x90.append(np.sqrt(a[0] ** 2 + a[1] ** 2))
                    qs2_y90.append(np.sqrt(a[2] ** 2 + a[3] ** 2))
                    qs2_z90.append(np.sqrt(a[4] ** 2 + a[5] ** 2))

        f2_90.close()
        qs2_x90 = np.asarray(qs2_x90)
        qs2_y90 = np.asarray(qs2_y90)
        trans = np.sqrt(qs2_x90 ** 2 + qs2_y90 ** 2)
        trans_90 = np.fliplr([trans])[0]
